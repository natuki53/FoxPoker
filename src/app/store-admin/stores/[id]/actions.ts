...
function errorRedirectPath(storeId: string, message: string) {
  return `/store-admin/stores/${storeId}?error=${encodeURIComponent(message)}`;
}

function successRedirectPath(storeId: string, key: string) {
  return `/store-admin/stores/${storeId}?saved=${encodeURIComponent(key)}`;
}
...
export async function updatePublicStoreProfile(formData: FormData) {
  ...
  try {
    ...
    redirect(errorRedirectPath(data.storeId, "更新に失敗しました。"));
  }

  revalidateStorePages(data.storeId);
  redirect(successRedirectPath(data.storeId, "profile"));
}

export async function updateStoreOperationalInfo(formData: FormData) {
  const session = await auth();
  if (!session?.user?.id) redirect("/auth/login?callbackUrl=/store-admin");
  ...
  try {
    ...
    const [activeTags, existingEvents] = await Promise.all([
      prisma.tag.findMany({
        where: { isActive: true },
        select: { id: true },
      }),
    ...
    for (const tagId of selectedTagIds) {
      if (!activeTagIds.has(tagId)) {
        throw new Error("タグの選択内容を確認してください。");
      }
    }
    ...
    await prisma.$transaction(async (tx) => {
      ...
      const currentTags = await tx.storeTag.findMany({
        where: { storeId: rawStoreId },
        select: { tagId: true },
      });
      const currentTagIds = new Set(currentTags.map((tag) => tag.tagId));

      const tagsToDelete = currentTags
      ...
      const tagsToCreate = selectedTagIds.filter((tagId) => !currentTagIds.has(tagId));
      if (tagsToCreate.length > 0) {
        await tx.storeTag.createMany({
          data: tagsToCreate.map((tagId) => ({ storeId: rawStoreId, tagId })),
          skipDuplicates: true,
        });
      }
    ...
    if (isRedirectError(error)) throw error;
    if (error instanceof Error) {
      redirect(errorRedirectPath(rawStoreId, error.message));
    }
    redirect(errorRedirectPath(rawStoreId, "詳細情報の更新に失敗しました。"));
  }

  revalidateStorePages(rawStoreId);
  redirect(successRedirectPath(rawStoreId, "details"));
}
...
export async function uploadStorePhoto(formData: FormData) {
  ...
  if (!(file instanceof File) || file.size === 0) {
    redirect(errorRedirectPath(rawStoreId, "画像ファイルを選択してください。"));
  }

  if (!ALLOWED_MIME_TYPES.has(file.type)) {
    redirect(
      errorRedirectPath(rawStoreId, "画像形式は JPG / PNG / WEBP / AVIF / GIF のみ対応です。")
    );
  }

  if (file.size > MAX_UPLOAD_SIZE) {
    redirect(errorRedirectPath(rawStoreId, "画像サイズは5MB以下にしてください。"));
  }

  const altText = typeof altTextRaw === "string" ? altTextRaw.trim() : "";
  if (altText.length > 120) {
    redirect(errorRedirectPath(rawStoreId, "代替テキストは120文字以内で入力してください。"));
  }
  ...
  try {
    ...
    redirect(errorRedirectPath(rawStoreId, "サムネイル画像のアップロードに失敗しました。"));
  }

  revalidateStorePages(rawStoreId);
  redirect(successRedirectPath(rawStoreId, "photos"));
}
...
export async function uploadStoreGalleryImage(formData: FormData) {
  ...
  try {
    ...
    redirect(errorRedirectPath(rawStoreId, "ギャラリー画像のアップロードに失敗しました。"));
  }

  revalidateStorePages(rawStoreId);
  redirect(successRedirectPath(rawStoreId, "gallery"));
}
...
export async function setMainStorePhoto(formData: FormData) {
  ...
  try {
    ...
    if (isRedirectError(error)) throw error;
    if (error instanceof Error) {
      redirect(errorRedirectPath(storeId, error.message));
    }
    redirect(errorRedirectPath(storeId, "メイン画像の更新に失敗しました。"));
  }

  revalidateStorePages(storeId);
  redirect(successRedirectPath(storeId, "photos"));
}
...
export async function moveStorePhoto(formData: FormData) {
  ...
  try {
    ...
    redirect(errorRedirectPath(storeId, "画像順序の更新に失敗しました。"));
  }
...
export async function moveStoreGalleryImage(formData: FormData) {
  ...
  try {
    ...
    if (swapIndex < 0 || swapIndex >= images.length) {
      redirect(successRedirectPath(storeId, "gallery"));
    }
    ...
    redirect(errorRedirectPath(storeId, "ギャラリー画像順序の更新に失敗しました。"));
  }
...
export async function deleteStorePhoto(formData: FormData) {
  ...
  try {
    ...
    redirect(errorRedirectPath(storeId, "画像の削除に失敗しました。"));
  }
...
export async function deleteStoreGalleryImage(formData: FormData) {
  ...
  try {
    ...
    redirect(errorRedirectPath(storeId, "ギャラリー画像の削除に失敗しました。"));
  }
...
export async function updateStorePublicPageContent(formData: FormData) {
  ...
  if (typeof rawJson !== "string") {
    redirect(errorRedirectPath(rawStoreId, "コンテンツデータが送信されませんでした。"));
  }
  ...
  try {
    ...
    redirect(errorRedirectPath(rawStoreId, "JSONの形式が正しくありません。"));
  }
  ...
  try {
    ...
    redirect(errorRedirectPath(rawStoreId, "お知らせの保存に失敗しました。"));
  }

  revalidateStorePages(rawStoreId);
  redirect(successRedirectPath(rawStoreId, "pagecontent"));
}
